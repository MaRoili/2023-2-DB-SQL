...
( lib_id VARCHAR(10) NOT NULL PRIMARY KEY,
  ...
  FOREIGN KEY(sector_id) REFERENCES sector(sector_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
( sector_id VARCHAR(10) NOT NULL PRIMARY KEY,
  ...
  FOREIGN KEY(lib_id) REFERENCES librarian(lib_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
( machine_id VARCHAR(10) NOT NULL PRIMARY KEY,
  ...
  FOREIGN KEY(dlv_id) REFERENCES delivery(dlv_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
( std_id VARCHAR(10) NOT NULL PRIMARY KEY,
  ...
  FOREIGN KEY(machine_id) REFERENCES selfs_machine(machine_id) ON UPDATE CASCADE ON DELETE CASCADE
  );
...
('S3','인문일반','L3'),
('S4','과학일반','L4');
 # 2023년 6월 1일 기준으로 도서 대출 시(checkout_sys에 데이터 삽입 시) 대출 도서 개수를 검색하고 3개 미만, 
 # 연체가 없는 경우에만 대출을 허용하는 프로시저 check_num_checkout

DELIMITER //
CREATE PROCEDURE check_num_checkout(
	IN user_id INT,
	IN book_id INT,
	IN checkout_id VARCHAR(10),
	IN std_id VARCHAR(10),
	IN date_keep DATE,
	IN date_ch DATE,
	IN date_prereturn DATE,
	IN date_return DATE,
	IN machine_id VARCHAR(10),
	IN price_overdue INT,
	IN times_extension INT,
	IN return_0 INT )
BEGIN
	DECLARE num_current_checkouts INT;
    DECLARE is_overdue BOOLEAN;
    
    SELECT COUNT(*) INTO num_current_checkouts
    FROM checkout_sys
    WHERE std_id = NEW.std_id AND return_0 = 0;
    
    IF num_current_checkouts <3 THEN
		SET is_overdue = date_prereturn < date_return(); 
		IF NOT is_overdue THEN
			INSERT INTO checkout_sys (book_id, checkout_id, std_id, date_keep, date_ch, date_prereturn, date_return, machine_id, price_overdue, times_extension, return_0)
			VALUES (NEW.book_id, NEW.checkout_id, NEW.std_id, NEW.date_keep, NEW.date_ch, NEW.date_prereturn, NEW.date_return, NEW.machine_id, NEW.price_overdue, NEW.times_extension, return_0);
		ELSE 
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '연체로 인해 대출이 불가합니다';
		END IF;
	ELSE 
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '대출이 불가합니다';
	END iF;
END //

DELIMITER ;

#  checkout_sys에 데이터가 삽입될 때 checkout_return 테이블에도 Insert가 발생하는 트리거
DELIMITER //
CREATE TRIGGER  AfterInsertSys
AFTER INSERT ON checkout_sys FOR EACH ROW
BEGIN
INSERT INTO checkout_return (std_id, checkout_id, book_id, date_keep, date_ch, date_prereturn, date_return, machine_id, price_overdue, times_extension, return_0)
VALUES (NEW.std_id, NEW.checkout_id, NEW.book_id, NEW.date_keep, NEW.date_ch, NEW.date_prereturn, NEW.date_return, NEW.machine_id, NEW.price_overdue, NEW.times_extension, NEW.return_0);
END ;
//
DELIMITER ;
...
#테스트 2
INSERT INTO checkout_sys (std_id, checkout_id, book_id, date_keep, date_ch, date_prereturn, date_return, machine_id, price_overdue, times_extension, return_0)
